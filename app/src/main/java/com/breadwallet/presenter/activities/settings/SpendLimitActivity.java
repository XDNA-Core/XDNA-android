...
public class SpendLimitActivity extends BRActivity {
    ...
    private void updateText(int progress) {
        ...
        BigDecimal amount = BRExchange.getAmountFromSatoshis(this, "XDNA", satoshis);
        ...
        String string = String.format("%s (%s)", BRCurrency.getFormattedCurrencyString(this, "XDNA", amount), BRCurrency.getFormattedCurrencyString(this, iso, curAmount));
    ...
    private BigDecimal getAmountBySte(int step) {
        ...
        switch (step) {
            case 0:
                result = new BigDecimal(ONE_BITCOIN * 500.0);//   500 XDNA
                break;
            case 1:
                result = new BigDecimal(ONE_BITCOIN * 1000.0);//   1000 XDNA
                break;
            case 2:
                result = new BigDecimal(ONE_BITCOIN * 5000.0);//   5000 XDNA
                break;
            case 3:
                result = new BigDecimal(ONE_BITCOIN * 10000.0);//   10000 XDNA
                break;

            default:
                result = new BigDecimal(ONE_BITCOIN * 500.0);//   500 XDNA Default
                break;
        }
        return result;
    }
    ...
    private int getStepFromLimit(long limit) {
        switch ((int) limit) {
            case ONE_BITCOIN * 500:
                return 0;
            case ONE_BITCOIN * 1000:
                return 1;
            case ONE_BITCOIN * 5000:
                return 2;
            case ONE_BITCOIN * 10000:
                return 3;
            default:
                return 0; //500 XDNA Default
        }
    }
